const { PL_OPERATOR_ROSTER } = require('./pl-operator-roster');
...
function getColumnOptions(sheet, title) {
    const column = (sheet.columns || []).find((candidate) => candidate.title === title);
    return (column?.options || []).filter((option) => String(option || '').trim());
}
...
function buildConfigPayload(sheet, options = {}) {
    ...
    const departments = [];
    const associateCellOptions = new Set();

    sheet.rows.forEach((row) => {
        const associateName = getCellByTitle(row, columnMap, 'Associate Name')?.trim();
        if (looksLikeAssociateName(associateName)) {
            const associateCell = getCellByTitle(row, columnMap, 'Cell') || '';
            if (String(associateCell).trim()) associateCellOptions.add(String(associateCell).trim());
            associates.push({
                id: row.id,
                name: associateName,
                Cell: associateCell,
        ...
        const operatorName = getCellByTitle(row, columnMap, 'Operators')?.trim();
        if (operatorName) {
            operators.push({ id: row.id, name: operatorName });
        }
    ...
    return {
        ...
        cellOptions: Array.from(new Set([
            ...getColumnOptions(sheet, 'Cell'),
            ...associateCellOptions
        ])),
        associates,
        operators: operators.length ? operators : PL_OPERATOR_ROSTER.map((name) => ({ id: null, name })),
        sequences,
        defects,
        events,
        inspectionParetos,
