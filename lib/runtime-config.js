...
function validateApplicationEnvironment(env = process.env) {
    ...
    if (parseBoolean(env.ALLOW_PTFE_MASTER_LOG_WRITES, false)) {
        required.push('DEPT_PTFE_API_TOKEN', 'DEPT_PTFE_MASTER_LOG_SHEET_ID');
    }
    if (parseBoolean(env.ALLOW_PI_MASTER_LOG_WRITES, false)) {
        required.push('DEPT_PI_API_TOKEN', 'DEPT_PI_MASTER_LOG_SHEET_ID');
    }
